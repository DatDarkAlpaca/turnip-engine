...
{
public:
	explicit CPU_ID(tur::U32 eax, tur::U32 ecx)
	{
		// Todo: write a cpuid function for linux/macos
		__cpuidex((int*)registers, eax, ecx);
	}

	explicit CPU_ID(tur::U32 eax)
	{
		__cpuid((int*)registers, eax);
	}

public:
	const tur::U32& eax() const { return registers[0]; }

	const tur::U32& ebx() const { return registers[1]; }

	const tur::U32& ecx() const { return registers[2]; }

	const tur::U32& edx() const { return registers[3]; }
